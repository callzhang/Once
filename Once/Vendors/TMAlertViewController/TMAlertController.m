...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(TMAlartControllerStyle)preferredStyle {
    ...
    if (self) {
        ...
        self.iconStyle = TMAlertControllerIconStyleQustion;
...
- (void)viewDidLoad {
	...
	if (self.preferredStyle == TMAlertControllerStyleTextField) {
		self.messageTextLabel.hidden = YES;
		self.editor.text = self.message;
	}else{
...
- (CGSize)preferredSizeFitsInWidth:(CGFloat)width {
	...
	CGFloat textFieldHeight = self.editor.frame.size.height;
    CGFloat titleLabelTop = 68;
    CGFloat labelSpacing = 10;
    CGFloat labelBottom = 88;
    
	return CGSizeMake(width, labelHeight1 + self.preferredStyle == TMAlertControllerStyleTextField ? labelHeight2:textFieldHeight + titleLabelTop + labelSpacing + labelBottom);
}

- (void)setIconStyle:(TMAlertControlerIconStyle)iconStyle {
    switch (iconStyle) {
        case TMAlertControllerIconStyleQustion: {
            self.iconImage = [UIImage imageNamed:@"eat-now-alert-question-mark-icon"];
            break;
        }
        case TMAlertControllerIconStyleThumbsUp: {
            self.iconImage = [UIImage imageNamed:@"eat-now-alert-thumbsup-icon"];
            break;
        }
        case TMAlertControllerIconStylePhone: {
            self.iconImage = [UIImage imageNamed:@"eat-now-alert-phone-icon"];
            break;
        }
		case TMAlertControllerIconStyleNote:{
			self.iconImage = [UIImage imageNamed:@"bubble"];
			break;
		}
        default: {
            break;
